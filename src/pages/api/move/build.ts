// pages/api/move/build.ts

/**
 * @swagger
 * /api/move/build:
 *   post:
 *     summary: Upload a Move project structure, build it, and clean up after build.
 *     tags:
 *       - Move
 *     requestBody:
 *       required: true
 ...
 *     responses:
 *       200:
 *         description: Build successful and project cleaned up.
 *       500:
 *         description: Failed to build or clean up.
 */
...
export default async function handler(
  ...
  async function forwardRequest(url: string): Promise<GasServerResponse> {
    try {
      const response = await fetch(`${url}/move/build`, {
        method: req.method,
        headers: {
